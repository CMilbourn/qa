#!/usr/bin/env python
# coding: utf-8
# qa_run_nophase_BGI.py based on qa_run_nophase.ipynb  
...
def process_data_nophase(imgm_cla, imgm_affine, core_filename, output_dir, mask_data=None, TR=None, nifti_path=None):
    ...
    # Handle NaN values before detrending
    if np.any(np.isnan(std_patch)) or np.any(np.isinf(std_patch)):
        print("Warning: NaN or Inf values found in std_patch, replacing with zeros")
    ...
    if np.any(np.isnan(average_patch)) or np.any(np.isinf(average_patch)):
        print("Warning: NaN or Inf values found in average_patch, replacing with zeros")
...
if __name__ == "__main__":
    # Location of data - Modified for BGI data
    mypathname = '/Users/cmilbourn/Documents/BGI_Data/BGI_Data_BIDS/sub008/sub008_visit001/'
    pathname_m = mypathname + 'func/'
    extension = '.nii.gz' #this can be .nii or .nii.gz

    # Search pattern for filenames - Modified to match BGI filename pattern
    filename_pattern = 'sub008_visit001_ses00fasted_task-rest_bold'
