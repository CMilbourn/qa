...
# Function to read MB/ARC/EchoNumber from BIDS JSON sidecar file
def get_accel_and_echo_from_json(nifti_path):
    """
    Read MultibandAccelerationFactor, ParallelReductionFactorInPlane,
    and EchoNumber from BIDS JSON sidecar file.

    Returns
    -------
    tuple
        (mb_factor, arc_factor, echo_number), each int or None.
    """
    json_path = nifti_path.replace('.nii.gz', '.json').replace('.nii', '.json')
    ...
    try:
        ...
        mb_factor = metadata.get('MultibandAccelerationFactor')
        arc_factor = metadata.get('ParallelReductionFactorInPlane')
        ...
        return (
            _to_int_or_none(mb_factor),
            _to_int_or_none(arc_factor),
            _to_int_or_none(echo_number),
        )
    except Exception as e:
        print(f"Error reading JSON file {json_path}: {e}")
        return None, None, None
...
def process_data_nophase(imgm_cla, imgm_affine, core_filename, output_dir, mask_data=None, TR=None, nifti_path=None, nifti_img=None):
    ...
    # Get MB / ARC / EchoNumber from JSON sidecar
    MB_factor, ARC_factor, Echo_number = (None, None, None)
    if nifti_path is not None:
        MB_factor, ARC_factor, Echo_number = get_accel_and_echo_from_json(nifti_path)
...
def create_combined_powerpoint(session_dir, all_output_dirs, all_metrics):
    ...
    for dataset_idx, output_dir in enumerate(all_output_dirs):
        ...
        if dataset_idx < len(all_metrics):
            ...
            # Table content
            rows = [
                ("MB", str(metrics.get('multiband_factor')) if metrics.get('multiband_factor') is not None else 'N/A'),
                ("ARC", str(metrics.get('arc_factor')) if metrics.get('arc_factor') is not None else 'N/A'),
                ("Echo", f"e{int(metrics.get('echo_number'))}" if isinstance(metrics.get('echo_number'), (int, float)) else 'N/A'),
    ...
    if all_metrics:
        ...
        try:
            key_metrics = [
                'filename', 'multiband_factor', 'arc_factor', 'echo_number',
                'num_timepoints', 'mean_isnr', 'mean_tsnr', 
            ...
            for col_idx, metric in enumerate(available_metrics):
                ...
                elif metric == 'multiband_factor':
                    metric_display = 'MB'
                elif metric == 'arc_factor':
                    metric_display = 'ARC'
                elif metric == 'echo_number':
                    metric_display = 'Echo'
            ...
            for row_idx, metrics in enumerate(all_metrics):
                for col_idx, metric in enumerate(available_metrics):
                    ...
                    elif metric in ('multiband_factor', 'arc_factor', 'echo_number'):
    ...
    # Add final ordered summary slide (sorted by MB, ARC, Echo)
    if all_metrics:
        blank_slide_layout = prs.slide_layouts[6]
        slide = prs.slides.add_slide(blank_slide_layout)
        ...
        txBox = slide.shapes.add_textbox(Inches(0.5), Inches(0.3), Inches(9), Inches(0.6))
        tf = txBox.text_frame
        tf.text = "QA Metrics Summary (Ordered by MB, ARC, Echo)"
        p = tf.paragraphs[0]
        p.font.size = Pt(16)
        p.font.bold = True
        p.alignment = PP_ALIGN.CENTER
        ...
        ordered_metrics = sorted(
            all_metrics,
            key=lambda m: (
                _as_int_or_none(m.get('multiband_factor')) if _as_int_or_none(m.get('multiband_factor')) is not None else 999,
                _as_int_or_none(m.get('arc_factor')) if _as_int_or_none(m.get('arc_factor')) is not None else 999,
                _as_int_or_none(m.get('echo_number')) if _as_int_or_none(m.get('echo_number')) is not None else 999,
                str(m.get('filename', '')).lower(),
            )
        )

        row_labels = ['MB', 'ARC', 'Echo', 'TE', 'tSNR', 'tSNR in Mask']
        table_rows = len(row_labels)
        table_cols = len(ordered_metrics) + 1
        ...
        for row_idx, label in enumerate(row_labels):
            ...
            label_par = table.cell(row_idx, 0).text_frame.paragraphs[0]
            label_par.font.bold = True
            label_par.font.size = Pt(10)
            label_par.alignment = PP_ALIGN.CENTER

        if len(ordered_metrics) > 0:
            data_col_width = max(0.08, (9.6 - 1.2) / len(ordered_metrics))
            for col_idx in range(1, table_cols):
                table.columns[col_idx].width = Inches(data_col_width)

        for col_idx, m in enumerate(ordered_metrics, start=1):
            mb = _as_int_or_none(m.get('multiband_factor'))
            arc = _as_int_or_none(m.get('arc_factor'))
            echo = _as_int_or_none(m.get('echo_number'))

            te_val = m.get('TE')
            tsnr_val = m.get('mean_tsnr')
            tsnr_mask_val = m.get('mean_tsnr_in_mask')

            table.cell(0, col_idx).text = str(mb) if mb is not None else 'N/A'
            table.cell(1, col_idx).text = str(arc) if arc is not None else 'N/A'
            table.cell(2, col_idx).text = f"e{echo}" if echo is not None else 'N/A'
            table.cell(3, col_idx).text = f"{float(te_val)*1000:.3f}" if isinstance(te_val, (int, float)) else 'N/A'
            table.cell(4, col_idx).text = f"{float(tsnr_val):.2f}" if isinstance(tsnr_val, (int, float)) else 'N/A'
            table.cell(5, col_idx).text = f"{float(tsnr_mask_val):.2f}" if isinstance(tsnr_mask_val, (int, float)) else 'N/A'

            for row_idx in range(table_rows):
                val_par = table.cell(row_idx, col_idx).text_frame.paragraphs[0]
                val_par.font.size = Pt(9)
                val_par.alignment = PP_ALIGN.CENTER
