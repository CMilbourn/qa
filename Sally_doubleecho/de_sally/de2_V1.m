...
if ~isempty(pathIn_e1)
  pathName_e1 = [pathIn_e1 filesep];
  fileName_e1 = [nameIn_e1 extIn_e1];
end
[pathIn_e2,nameIn_e2,extIn_e2] = fileparts(fileName_e2);
if ~isempty(pathIn_e2)
  pathName_e2 = [pathIn_e2 filesep];
  fileName_e2 = [nameIn_e2 extIn_e2];
end
...
if ~isempty(regexpi(fileName_e1, '\.nii\.gz$', 'once'))
  ...
  if ~exist(fullfile(pathName_e1, fileName_e1_unzipped), 'file')
    gunzip(fullfile(pathName_e1, fileName_e1));
  end
  fileName_e1 = fileName_e1_unzipped;
end
if ~isempty(regexpi(fileName_e2, '\.nii\.gz$', 'once'))
  fileName_e2_unzipped = regexprep(fileName_e2, '\.gz$', '');
  if ~exist(fullfile(pathName_e2, fileName_e2_unzipped), 'file')
    gunzip(fullfile(pathName_e2, fileName_e2));
  end
  fileName_e2 = fileName_e2_unzipped;
end

%% Read in selective images
% Open the raw data for analysis - echo_1
disp('Opening file - echo1');
filePath_e1 = fullfile(pathName_e1,fileName_e1);
...
filePath_e2 = fullfile(pathName_e2,fileName_e2);
